...
$query = "SELECT * FROM tasks WHERE id_task = $id_task AND id_user = $id_user";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $priority = mysqli_real_escape_string($conn, $_POST['task-priority'] ?? 'rendah');
    $status = mysqli_real_escape_string($conn, $_POST['task-status'] ?? 'belum_dikerjakan');
    $deadline = mysqli_real_escape_string($conn, $_POST['task-due'] ?? '');
    ...
    if ($status == 'todo') $status = 'belum_dikerjakan';
    elseif ($status == 'in-progress') $status = 'sedang_dikerjakan';
...
<div class="mb-8">
<a href="tasks.php" class="text-on-surface-variant flex items-center gap-1 font-label-md text-label-md hover:text-primary transition-colors mb-4 inline-flex">
<span class="material-symbols-outlined text-[16px]">arrow_back</span>
                    Back to Tasks
                </a>
<h1 class="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface mb-2">Edit Task: <?= htmlspecialchars($task['title']) ?></h1>
<p class="font-body-md text-body-md text-on-surface-variant">Update the details, priority, and deadline for your objective.</p>
</div>
...
<?php foreach ($categories as $cat): ?>
    <option value="<?= $cat['id_category'] ?>" <?= $task['id_category'] == $cat['id_category'] ? 'selected' : '' ?>><?= htmlspecialchars($cat['category_name']) ?></option>
<?php endforeach; ?>
</select>
...
<input <?= $task['priority'] == 'rendah' ? 'checked' : '' ?> class="peer sr-only" name="task-priority" type="radio" value="low"/>
...
<label class="flex-1 cursor-pointer">
<input <?= $task['priority'] == 'sedang' ? 'checked' : '' ?> class="peer sr-only" name="task-priority" type="radio" value="medium"/>
...
<label class="flex-1 cursor-pointer">
<input <?= $task['priority'] == 'tinggi' ? 'checked' : '' ?> class="peer sr-only" name="task-priority" type="radio" value="high"/>
...
<!-- Deadline -->
<div>
<label class="block font-label-md text-label-md text-on-surface mb-2" for="task-due">Deadline</label>
<div class="relative">
<input required class="w-full h-[40px] pl-4 pr-10 bg-surface rounded-lg border border-border text-on-surface font-body-md text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all cursor-pointer" id="task-due" name="task-due" type="date" value="<?= $task['deadline'] ?>"/>
<div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-on-surface-variant">
...
<!-- Status -->
<div>
<label class="block font-label-md text-label-md text-on-surface mb-2" for="task-status">Status</label>
<div class="relative">
<select name="task-status" class="w-full h-[40px] px-4 appearance-none bg-surface rounded-lg border border-border text-on-surface font-body-md text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all cursor-pointer" id="task-status">
<option value="todo" <?= $task['status'] == 'belum_dikerjakan' ? 'selected' : '' ?>>To Do</option>
<option value="in-progress" <?= $task['status'] == 'sedang_dikerjakan' ? 'selected' : '' ?>>In Progress</option>
<option value="done" <?= $task['status'] == 'selesai' ? 'selected' : '' ?>>Done</option>
</select>
<div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-on-surface-variant">
<span class="material-symbols-outlined text-[20px]">expand_more</span>
</div>
...
<a href="tasks.php" class="px-6 py-2 rounded-lg font-label-md text-label-md text-on-surface border border-transparent hover:border-border hover:bg-surface-container-low transition-colors">Cancel</a>
<button class="px-6 py-2 rounded-lg font-label-md text-label-md bg-primary-container text-on-primary hover:bg-primary shadow-sm hover:shadow-md transition-all active:scale-[0.98]" type="submit">Update Task</button>
</div>
</form>
</div>
</div>
...
<h3 class="font-headline-md text-headline-md text-on-surface mb-4">Edit Tips</h3>
...
<span>Updating priority helps team members understand task urgency.</span>
</li>
<li class="flex items-start gap-3">
<span class="material-symbols-outlined text-status-todo text-[20px] shrink-0 mt-0.5">update</span>
<span>Ensure the deadline accurately reflects any recent project changes.</span>
</li>
</ul>
</div>
</div>
...
<nav class="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-3 bg-surface dark:bg-inverse-surface shadow-lg border-t border-outline-variant dark:border-outline docked full-width bottom-0 rounded-t-xl shadow-md">
<a class="flex flex-col items-center justify-center text-on-surface-variant dark:text-surface-variant active:bg-surface-container-highest Tap: scale-90 duration-150 p-2 rounded-lg group" href="dashboard.php">
<span class="material-symbols-outlined mb-1">home</span>
<span class="font-label-sm text-label-sm-mobile">Home</span>
</a>
<a class="flex flex-col items-center justify-center text-primary dark:text-inverse-primary font-bold active:bg-surface-container-highest Tap: scale-90 duration-150 p-2 rounded-lg group" href="tasks.php">
<span class="material-symbols-outlined mb-1" style="font-variation-settings: 'FILL' 1;">list_alt</span>
<span class="font-label-sm text-label-sm-mobile">Tasks</span>
</a>
<a class="flex flex-col items-center justify-center text-on-surface-variant dark:text-surface-variant active:bg-surface-container-highest Tap: scale-90 duration-150 p-2 rounded-lg group" href="add_task.php">
<span class="material-symbols-outlined mb-1">add_circle</span>
<span class="font-label-sm text-label-sm-mobile">Add</span>
</a>
<a class="flex flex-col items-center justify-center text-on-surface-variant dark:text-surface-variant active:bg-surface-container-highest Tap: scale-90 duration-150 p-2 rounded-lg group" href="#">
